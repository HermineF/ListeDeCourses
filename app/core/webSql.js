	...
	*/
	...
	function initializeData(callback){
		...
		$.each(Application.Model.getModel(),function(index,model){
			queriesList.push(new Query("DELETE FROM "+index));
			$.each(model,function(propertyName,property){
				if(typeof(Application.Types[property.type]) === "undefined"
					...
					queriesList.push(new Query("DELETE FROM "+index+"_"+property.type));
				}
			});
		});
	...
	function getInsertQueries(modelName, values,isLocallyModified){
		...
		if(typeof(model) !== "undefined"){
			...
			console.warn("Le model "+modelName+" n'existe pas les données ne seront pas insérées");
		}
		return queries;
	}
	...
	this.getModelData = function(modelName,callback){
		var queries = [];
		queries.push(new Query("SELECT * FROM "+modelName));
